...
public sealed class MainForm : Form
{
    ...
    private AppLanguage currentLanguage = AppLanguage.Chinese;
    private DnsMode currentMode = DnsMode.DetectionFailed;

    public MainForm()
    {
        currentLanguage = AppConfig.Load(configPath).AppLanguage;
        Text = UiText.Title(currentLanguage);
    ...
    private void BuildLayout()
    {
        ...
        descriptionLabel.ForeColor = Color.FromArgb(70, 78, 88);
        descriptionLabel.TextAlign = ContentAlignment.MiddleLeft;
        descriptionLabel.AutoEllipsis = true;
        descriptionLabel.Margin = new Padding(0, 0, 0, 8);
        ...
        adapterPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 150));
        adapterPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 72));
        adapterPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 118));
        ...
        languageLabel.Margin = new Padding(0, 2, 8, 2);
        ...
        adapterPanel.Controls.Add(languageLabel, 3, 2);
        adapterPanel.Controls.Add(languageComboBox, 4, 2);
        adapterPanel.SetColumnSpan(adapterComboBox, 5);
        adapterPanel.SetColumnSpan(selectedAdapterLabel, 5);
        ...
        ConfigureMainButton(automaticDnsModeButton);
        ConfigureMainButton(customDnsModeButton);
    ...
    private void WireEvents()
    {
        ...
        languageComboBox.SelectedIndexChanged += (_, _) => LanguageSelectionChanged();
        ...
        browserLeaksButton.Click += (_, _) => StartShell("https://browserleaks.com/ip", UiText.OpenDiagnosticFailed(currentLanguage));
        ipLeakButton.Click += (_, _) => StartShell("https://ipleak.net", UiText.OpenDiagnosticFailed(currentLanguage));
        claudeIpCheckButton.Click += (_, _) => StartShell("https://ip.net.coffee/", UiText.OpenDiagnosticFailed(currentLanguage));
        scamalyticsButton.Click += (_, _) => StartShell("https://scamalytics.com/ip", UiText.OpenDiagnosticFailed(currentLanguage));
        ping0Button.Click += (_, _) => StartShell("https://ping0.cc", UiText.OpenDiagnosticFailed(currentLanguage));
        adapterInfoButton.Click += (_, _) => ShowAdapterInfo();
    }

    private static void ConfigureMainButton(Button button)
    {
    ...
    private void ApplyLanguage()
    {
        ...
        {
            Text = UiText.Title(currentLanguage);
            titleLabel.Text = UiText.Title(currentLanguage);
            descriptionLabel.Text = UiText.Subtitle(currentLanguage);
            refreshAdaptersButton.Text = UiText.RefreshAdapters(currentLanguage);
            showAllAdaptersCheckBox.Text = UiText.ShowAllAdapters(currentLanguage);
            languageLabel.Text = UiText.LanguageLabel(currentLanguage);
            automaticDnsModeButton.Text = UiText.AutomaticDnsButton(currentLanguage);
            customDnsModeButton.Text = UiText.CustomDnsButton(currentLanguage);
            customDnsSettingsButton.Text = UiText.SetCustomDns(currentLanguage);
            flushDnsButton.Text = UiText.RefreshDns(currentLanguage);
            networkConnectionsButton.Text = UiText.NetworkConnections(currentLanguage);
            adapterInfoButton.Text = UiText.AdapterInfo(currentLanguage);
            languageComboBox.SelectedItem = languageComboBox.Items
                .OfType<LanguageOption>()
                .FirstOrDefault(option => option.Language == currentLanguage);
            SetStatus(SelectedAdapter, currentMode);
        }
        finally
        {
    ...
    {
        if (isApplyingLanguage || languageComboBox.SelectedItem is not LanguageOption option)
        {
            return;
        }

        currentLanguage = option.Language;
        SaveConfig(AppConfig.Load(configPath).WithLanguage(currentLanguage));
        ApplyLanguage();
    }
    ...
    private void LoadAdapters()
    {
        ...
        try
        {
            ...
            if (adapterComboBox.Items.Count == 0)
            {
                ...
                AddLog(UiText.NoWirelessAdapter(currentLanguage));
            }
        }
        catch (Exception ex)
        {
            AddLog(UiText.AdapterReadFailed(currentLanguage, ex.Message));
        }
        finally
        {
    ...
    private async Task SwitchAutomaticDnsModeAsync()
    {
        ...
        if (adapter is null)
        {
            AddLog(UiText.SelectAdapterFirst(currentLanguage));
            return;
        }
        ...
        {
            var message = UiText.AlreadyAutomaticDns(currentLanguage);
            AddLog(message);
            MessageBox.Show(message, Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }

        await RunCommandsAsync(DnsCommandFactory.CreateAutomaticDnsModeCommands(adapter.Name), UiText.SwitchedToAutomaticDns(currentLanguage));
    }
    ...
    private async Task SwitchCustomDnsModeAsync()
    {
        ...
        {
            var message = UiText.SetCustomDnsFirst(currentLanguage);
            AddLog(message);
            MessageBox.Show(message, Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }
        ...
        {
            MessageBox.Show(UiText.InvalidIpv4Dns(currentLanguage), Text, MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
        ...
        {
            var message = UiText.AlreadyCustomDns(currentLanguage);
            AddLog(message);
            MessageBox.Show(message, Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }

        await RunCommandsAsync(DnsCommandFactory.CreateCustomDnsModeCommands(adapter.Name, config.CustomDnsSettings), UiText.SwitchedToCustomDns(currentLanguage));
    }
    ...
    private async Task ShowCustomDnsSettingsDialogAsync()
    {
        ...
        {
            Text = UiText.SetCustomDns(currentLanguage),
        ...
        {
            Text = UiText.CustomDnsDialogHelp(currentLanguage),
            Dock = DockStyle.Fill,
        ...
        {
            Text = UiText.PrimaryDns(currentLanguage),
            Dock = DockStyle.Fill,
        ...
        {
            Text = UiText.SecondaryDns(currentLanguage),
            Dock = DockStyle.Fill,
            TextAlign = ContentAlignment.MiddleLeft,
        };
        ...
        {
            Text = UiText.Save(currentLanguage),
            Width = 86,
            Height = 32,
        ...
        {
            Text = UiText.Cancel(currentLanguage),
            Width = 86,
            Height = 32,
        ...
        saveButton.Click += (_, _) =>
        {
            ...
            {
                MessageBox.Show(UiText.FillPrimaryDns(currentLanguage), dialog.Text, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (validation == CustomDnsValidationStatus.InvalidAddress)
            {
                MessageBox.Show(UiText.InvalidIpv4Dns(currentLanguage), dialog.Text, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            SaveConfig(AppConfig.Load(configPath)
                ...
                .WithShowAllAdapters(showAllAdaptersCheckBox.Checked));
            var message = UiText.DnsSettingsSaved(currentLanguage);
            AddLog(message);
            MessageBox.Show(message, dialog.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
    ...
    private async Task FlushDnsAsync(bool showDialog)
    {
        ...
        if (result.Succeeded)
        {
            AddLog(UiText.DnsRefreshed(currentLanguage));
            if (showDialog)
            {
                MessageBox.Show(UiText.DnsRefreshed(currentLanguage), Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            return;
        }

        ShowFailure(UiText.RefreshDnsFailed(currentLanguage), result);
    }
    ...
    private async Task RunCommandsAsync(IEnumerable<CommandSpec> commands, string successMessage)
    {
        ...
        try
        {
            foreach (var command in commands)
            {
                ...
                if (!result.Succeeded)
                {
                    ShowFailure(UiText.OperationFailed(currentLanguage), result);
                    return;
                }
            }
    ...
    {
        if (AdapterClassifier.IsHighRiskVirtualAdapter(adapter))
        {
            return MessageBox.Show(
                UiText.VirtualAdapterWarning(currentLanguage),
                Text,
        ...
        {
            return MessageBox.Show(
                UiText.NonWirelessAdapterWarning(currentLanguage),
                Text,
                MessageBoxButtons.YesNo,
                MessageBoxIcon.Warning) == DialogResult.Yes;
        }
    ...
    {
        var confirmation = ModeSwitchConfirmation.ForMode(targetMode, adapter.Name, currentLanguage);
        return MessageBox.Show(
    ...
    private void ShowFailure(string title, CommandResult result)
    {
        var detail = string.IsNullOrWhiteSpace(result.CombinedOutput)
            ? UiText.FailureWithExitCode(currentLanguage, result.ExitCode)
    ...
    {
        currentMode = mode;
        selectedAdapterLabel.Text = adapter is null ? UiText.NoAdapterSelected(currentLanguage) : UiText.CurrentAdapter(currentLanguage, adapter.DisplayName);
        modeLabel.Text = NetworkInfoFormatter.FormatMode(mode, currentLanguage);
        ipv6Label.Text = NetworkInfoFormatter.FormatIpv6Status(adapter, currentLanguage);
    ...
    private void ShowAdapterInfo()
    {
        ...
        if (adapter is null)
        {
            MessageBox.Show(UiText.SelectAdapterFirst(currentLanguage), Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }

        MessageBox.Show(NetworkInfoFormatter.FormatAdapterInfo(adapter, currentLanguage), UiText.AdapterInfo(currentLanguage), MessageBoxButtons.OK, MessageBoxIcon.Information);
    }

    private void StartShell(string target, string? failureTitle = null)
    {
        try
        {
        ...
        {
            failureTitle ??= UiText.OpenFailed(currentLanguage);
